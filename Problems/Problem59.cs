...
namespace Euler.Solutions
{
    ...
    class Problem59: IProblem
    {
        ...
        {
            var input = get(ciphertxt);
            var lijsten = from c in input.Select((item, index) => new { item, index })
        ...
        /// <summary>
        /// This function returns a character a - z which has the maximum count in a frequency table.
        /// </summary>
        ...
        /// <summary>
        /// This function counts valid characters when decrypting the input with a to z.
        /// </summary>
        private static IEnumerable<int> Validate(byte[] input)
        {
            var valid = from x in Enumerable.Range('a', 26)
                        select input.Count(c => ((c ^ x) >= 'a' && (c ^ x) <= 'z'));
            return valid;
        }
        ...
        {
            return (from s in ciphertxt.Split(',') select (byte)int.Parse(s)).ToArray();
        }
